...
// InitializeDuel initializes a new duel on-chain with player 1's deposit
func (c *AnchorClient) InitializeDuel(
	ctx context.Context,
	duelID uint64,
	amount uint64,
	predictedOutcome uint8,
	...
	// Discriminator (8 bytes) + duel_id (8 bytes) + amount (8 bytes) + predicted_outcome (1 byte)
	data := make([]byte, 25)

	// Discriminator from IDL: [197, 5, 158, 89, 174, 188, 134, 6]
	discriminator := []byte{197, 5, 158, 89, 174, 188, 134, 6}
	copy(data[0:8], discriminator)

	// duel_id: u64 (little-endian)
	binary.LittleEndian.PutUint64(data[8:16], duelID)

	// amount: u64 (little-endian)
	binary.LittleEndian.PutUint64(data[16:24], amount)

	// predicted_outcome: u8
	data[24] = predictedOutcome

	// Build accounts
	accounts := []*solana.AccountMeta{
		{PublicKey: duelPDA, IsWritable: true, IsSigner: false},                 // duel
		{PublicKey: player1Pubkey, IsWritable: true, IsSigner: true},            // player_1
	...
	tx, err := solana.NewTransaction(
		...
		solana.TransactionPayer(player1Pubkey),
	)
	if err != nil {
		return "", fmt.Errorf("failed to create transaction: %w", err)
	}

	// Get player 1 private key from environment
