...
func (dr *DuelResolver) resolveExpiredDuels() {
	...
	for _, duel := range duels {
		...
		// For now, use a simple approach - in production, get actual price data
		winnerID, exitPrice, err := dr.determineWinner(ctx, duel)
		if err != nil {
			log.Printf("[DuelResolver] Error determining winner for duel %s: %v", duel.ID, err)
			continue
		}

		// Resolve the duel
		_, err = dr.duelService.ResolveDuelWithPrice(
			ctx,
			duel.ID,
			winnerID,
			exitPrice,
			"", // txHash - will be generated by ResolveDuelWithPrice
		)
		if err != nil {
		...
		resolvedCount++
		log.Printf("[DuelResolver] Successfully resolved duel %s, winner: %d", duel.ID, winnerID)
	}
